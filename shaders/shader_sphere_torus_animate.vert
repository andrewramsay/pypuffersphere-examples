...
uniform float torus_rotate;
uniform float time_clock;
...
float snoise(vec3 co){
    return fract(sin(dot(co.xyz ,vec3(12.9898,78.233,-31.093831))) * 43758.5453);
}
...
{   
    ...
    float ph1 = snoise(gl_Normal);
    float ph2 = snoise(gl_Normal+vec3(3003031,0130,102.3));
    float ph3 = snoise(gl_Normal+vec3(5899,03,031.455));
    norm_pos.x += sin(time_clock*(10+ph1)-1.4)*0.05;
    norm_pos.y += cos(time_clock*(10+ph2)+0.95)*0.05;
    norm_pos.z += sin(time_clock*(10+ph3)+0.42)*0.05;
